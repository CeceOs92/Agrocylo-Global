...
export class WsServer {
  ...
  private handleClientMessage(socket: WebSocket, data: RawData): void {
    ...
    try {
      const payload = jwt.verify(msg.token, config.jwtSecret) as {
    ...
    for (const client of this.wss.clients) {
      ...
      if (queue) {
        if (queue.length >= this.maxQueueDepth) {
          queue.shift();
          logger.warn(
            'WebSocket send queue exceeded, dropping oldest message',
            {
  ...
  private flushQueue(client: WebSocket, type: WsEventType): void {
    ...
    client.send(message, (error: Error | undefined) => {
      if (error) {
        logger.debug('WebSocket delivery failed', {
          type,
          error: error.message,
        });
      } else {
  ...
  drain(): Promise<void> {
    return new Promise((resolve) => {
      ...
      const timeout = setTimeout(() => {
        logger.warn(
          'WebSocket drain timed out, terminating remaining connections',
        );
